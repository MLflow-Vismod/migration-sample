...
.. delta-edge-only::
  .. admonition::  Experimental
      ...
      This is an [experimental](https://docs.databricks.com/release-notes/release-types.html#databricks-runtime-preview-releases) integration developed by the <Delta> open-source community. Use with caution.
...
.. azure::
  ...
  <a id="azure-compatibility"></a>

  ### Compatibility with Azure file systems

  When using Azure file systems (Azure Blob storage, Azure Data Lake Storage (Gen1/Gen2)) for Delta tables, it is possible to get incorrect results when <Snowflake> concurrently queries the manifest while the manifest files are being rewritten. Known issues regarding the atomic visibility of files in the Azure file systems that can make the manifest files to be momentarily unavailable. Hence, use manifests with caution if their updates are likely to coincide with queries from <Snowflake>.
...
.. delta-oss-only::
  .. <cloud-path> replace:: storage path
  .. <require cloud-paths> replace:: require paths
...
.. azure::
  .. <cloud-path> replace:: Azure Blob storage or Azure Data Lake Storage (Gen1/Gen2) path
  .. <require cloud-paths> replace:: requires Azure Blob storage or Azure Data Lake Storage (Gen1/Gen2) paths
  .. <DatabricksOrSpark> replace:: <Databricks>
  .. <DBROrSpark> replace:: <DBR>
