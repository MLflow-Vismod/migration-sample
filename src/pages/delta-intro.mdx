---
description: Learn about <DeltaLake /> features and resources to learn about <DeltaLake />.
---
...
<DeltaLake /> provides [ACID transactions](concurrency-control.md), scalable metadata
handling, and unifies [streaming](delta-streaming.md) and
[batch](delta-batch.md) data processing on top of existing data lakes, such as S3, ADLS, GCS, and HDFS.



For a quick overview and benefits of <DeltaLake />, watch this YouTube video (3
minutes).
...
- [Streaming](delta-streaming.md) and [batch](delta-batch.md) unification: A
  table in <DeltaLake /> is a batch table as well as a streaming source and sink.
...
- [Upserts](delta-update.md#delta-merge) and
  ...
  For a general introduction and demonstration of <DeltaLake />, watch this YouTube
  video (51 minutes).
  ...
  Delta Engine optimizations make <DeltaLake /> operations highly performant,
  ...
  The <DeltaLake /> quickstart provides an overview of the basics of working with
  <DeltaLake />. The [quickstart](quick-start.md) shows how to load data into a Delta
  table, modify the table, read the table, display table history, and optimize
  the table.

  For <Databricks> notebooks that demonstrate these features, see
  [_](intro-notebooks.md).

  .. aws::

    To try out <DeltaLake />, see [Sign up for Databricks](/getting-started/try-databricks.md).

  .. azure::

    To try out <DeltaLake />, see [Sign up for Azure Databricks](https://docs.microsoft.com/azure/azure-databricks/quickstart-create-databricks-workspace-portal).

  .. gcp::

    To try out <DeltaLake />, see [Sign up for Databricks](/getting-started/try-databricks-gcp.md).

  ## Key tasks

  The following list provides links to documentation for common <DeltaLake /> tasks.
  ...
  - For reference information on <DeltaLake /> SQL commands, see
  ...
  For deep-dive training on <DeltaLake />, watch this YouTube video (2 hours, 42
  minutes).
